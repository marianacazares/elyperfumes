...
//Elementos DOM
const caballeroSection = document.querySelector('#caballero');
document.addEventListener('DOMContentLoaded', async function () {
    const q = query(collection(db, "articulosCatalogo"), where("categoriaArt", "==","CABALLERO"));
    const querySnapshot = await getDocs(q);
    querySnapshot.forEach((doc) => {
    console.log(doc.id, " => ", doc.data())
    caballeroSection.innerHTML += `
        <div class="articulo">
